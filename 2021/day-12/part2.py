...
def solve(nodes: list[Node], current_node: Node, current_path: list[Node]):
    ...
    if current_node.isEnd:
        ...
        for node in current_node.linked_nodes:
            if isValidMove(current_path, node): # replace
                solve(nodes, node, current_path)

def isValidMove(path: list[Node], node: Node):
    if node.name == "start":
        return False
    if node.isBig or node not in path:
        return True
    return not hasTwoSmalls(path)
    
def hasTwoSmalls(path: list[Node]):
    smalls = []
    two_smalls = False
    for node in path:
        if node.isSmall and node.name in smalls:
            two_smalls = True
        smalls.append(node.name)
    return two_smalls
